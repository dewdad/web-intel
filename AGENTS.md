...
2. Checks `.deps_cache/{hash}.stamp` — if fresh, skips entirely
3. If Python ≥ 3.14, warns and tries to use an older Python for pip installs
4. If missing/stale, verifies each package can be imported (via `_IMPORT_MAP`)
5. Installs missing packages with `pip install --quiet --upgrade`
6. Writes stamp file on success

To force re-check: `rm -rf .deps_cache/`

## Conventions

- No tests exist. Verify changes by running: `bin/web-intel doctor --pretty` for full health check, or `bin/web-intel <command> --help` for parse check, actual URLs for integration check.
